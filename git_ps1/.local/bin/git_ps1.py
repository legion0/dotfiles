...
p = Popen(['git', 'symbolic-ref', 'HEAD'], stdout=PIPE, stderr=PIPE, text=True)
...
stdoutdata, stderrdata = Popen(['git', 'rev-parse',  '--show-toplevel'], stdout=PIPE, stderr=PIPE, text=True).communicate()
if stderrdata != "":
	_die("BAD -2", -2)
gitRoot = stdoutdata.strip()

stdoutdata, stderrdata = Popen(['git','status','--porcelain'], stdout=PIPE, stderr=PIPE, text=True).communicate()
...
if not branch: # not on any branch
	...
	remote_name = Popen(['git','config','branch.%s.remote' % branch], stdout=PIPE, text=True).communicate()[0].strip()
	if remote_name:
		merge_name = Popen(['git','config','branch.%s.merge' % branch], stdout=PIPE, text=True).communicate()[0].strip()
	else:
		remote_name = "origin"
	...
	revgit = Popen(['git', 'rev-list', '--left-right', '%s...HEAD' % remote_ref],stdout=PIPE, stderr=PIPE, text=True)
	revlist = revgit.communicate()[0]
	if revgit.poll(): # fallback to local
		revlist = Popen(['git', 'rev-list', '--left-right', '%s...HEAD' % merge_name],stdout=PIPE, stderr=PIPE, text=True).communicate()[0]
